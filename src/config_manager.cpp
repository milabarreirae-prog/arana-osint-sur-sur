...
bool ConfigManager::cargar_configuracion(const std::filesystem::path& ruta_json) {
    std::lock_guard<std::mutex> cerrojo(mutex_);
    
    if (!std::filesystem::exists(ruta_json)) {
        spdlog::warn("ConfigManager: No se encontró {}. Usando defaults.", ruta_json.string());
        return false;
    }
    ...
    try {
        ...
        spdlog::info("ConfigManager: SSOT JSON cargado exitosamente desde {}", ruta_json.string());
        return true;
    } catch (const nlohmann::json::exception& e) {
        spdlog::error("ConfigManager: Error crítico de parseo JSON en {}: {}", ruta_json.string(), e.what());
        return false;
    } catch (const std::exception& e) {
        spdlog::error("ConfigManager: Error de I/O en {}: {}", ruta_json.string(), e.what());
        return false;
    }
}
...
void ConfigManager::parsing_json_to_structs_(const nlohmann::json& j) {
    ...
    try {
        if (j.contains("network")) {
            ...
            network_.user_agent = j["network"].value("user_agent", "SursurSpider/1.0");
            network_.strict_sur_sur_filter = j["network"].value("strict_sur_sur_filter", true);
        }
        if (j.contains("sources")) {
            sources_.contrahegemonic_seeds = j["sources"].value("contrahegemonic_seeds", std::vector<std::string>{});
            sources_.hegemonic_contrast_seeds = j["sources"].value("hegemonic_contrast_seeds", std::vector<std::string>{});
            sources_.target_keywords = j["sources"].value("target_keywords", std::vector<std::string>{});
        }
        if (j.contains("llm")) {
            llm_.model_name = j["llm"].value("model_name", "qwen2.5-coder:7b");
            llm_.endpoint = j["llm"].value("endpoint", "http://localhost:11434/api/generate");
        ...
        if (j.contains("persistence")) {
            persistence_.sqlite_path = j["persistence"].value("sqlite_path", "/var/lib/sursur/db/graph.db");
        }
        if (j.contains("media")) {
        ...
        spdlog::error("ConfigManager: Fallo al extraer variables del JSON: {}", e.what());
    }
}
...
void ConfigManager::volcar_configuracion() const {
    ...
    spdlog::info("[Semillas] Contrahegemónicas: {}, Hegemónicas: {}", 
                 sources_.contrahegemonic_seeds.size(), sources_.hegemonic_contrast_seeds.size());
    spdlog::info("[Persistencia] SQLite en: {}", persistence_.sqlite_path.string());
}
